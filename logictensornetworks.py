...
def variable(label,number_of_features_or_feed):
    if type(number_of_features_or_feed) is int:
        ...
        result = tf.constant(np.array(number_of_features_or_feed).astype('float32'),name=label)
    result.doms = [label]
    return result
...
def constant(label,value=None,
    ...
    if value is not None:
        result = tf.constant(np.array(value).astype('float32'),name=label)
    else:
...
def proposition(label,initial_value=None,value=None):
    if value is not None:
        ...
        result = tf.expand_dims(tf.clip_by_value(tf.Variable(tf.random.normal(shape=(),mean=.5,stddev=.5)),0.,1.),dim=0)
    result.doms = ()
    return result

...
def initialize_tf_variables(all=False):
    if tf.compat.v1.global_variables():
        if all:
            init = tf.compat.v1.global_variables_initializer()
        else:
            uninitialized_variable_names=set([v.decode("ascii") for v in SESSION.run(tf.report_uninitialized_variables())])
            uninitialized_variables=[var for var in tf.compat.v1.global_variables() if var.name.split(":")[0] in uninitialized_variable_names]
...
def initialize_knowledgebase(optimizer=None,
    formula_aggregator=lambda *x: 1/tf.reduce_mean(1/tf.concat(x,axis=0)) if x else None,
    ...
    if tf.compat.v1.global_variables() and KNOWLEDGEBASE is not None:
...
def ask(term_or_formula,feed_dict={}):
    ...
    if _t is None:
        ...
        _feed_dict=_compute_feed_dict(feed_dict)
        _uninit_vars_name = SESSION.run(tf.report_uninitialized_variables())
        _uninit_vars = [var for var in tf.global_variables() if str.encode(var.op.name) in _uninit_vars_name]
        SESSION.run(tf.variables_initializer(_uninit_vars))
